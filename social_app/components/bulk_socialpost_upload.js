...
// COMPONENT DID MOUNT
	...
	render() {
		...
		if ( this.state.switchScreen !== false ){
			...
			// redirecting
			this.props.navigation.navigate('SocialPost', {
				itemId: 86,
			...
			// e.g a social post, textinput which lets user to enter text, takes persons id as assigned object
				<View style={styles.outerContainer}>
					...
					<View style={styles.textinputContainer}>
						<Button 
							title={'Select SOCIAL POST IMAGES From Phone'}
							...
							onPress={async () => {
								try {
									...
									this.setState(prev => ({...prev, socialpost_image: results}))
									// results.map((res) => {
					...
					<View style={styles.textinputContainer}>
						<Button 
							title={'Select SOCIAL POST VIDEOS From Phone'}
							style={styles.buttonWithoutBG}
							onPress={async () => {
								try {
									const results = await DocumentPicker.pickMultiple({
									...
									// setState method with response as argument
									this.setState(prev => ({...prev, socialpost_video: results}))
									// results.map((res) => {
										// console.log(res.uri, res.type, res.name, res.size); // res.type is mimeType
									// })
								} catch (err) {
					...
					<Button 
						...
						onPress={ () => {
							...
							Array.from(this.state.socialpost_image).forEach((file) => {
								formData.append('socialpost_image', {uri: file.uri, name: file.name, type: file.type})
							})
							Array.from(this.state.socialpost_video).forEach((file) => {
								formData.append('socialpost_video', {uri: file.uri, name: file.name, type: file.type})
							})
							formData.append('excel_sheet_for_socialpost', {uri: this.state.excel_sheet.uri, name: this.state.excel_sheet.name, type: this.state.excel_sheet.type})
...
export default BulkSocialpostUpload