			...
			// e.g a social post, textinput which lets user to enter text, takes persons id as assigned object
				<View style={styles.outerContainer}>
					...
					<View style={styles.textinputContainer}>
						<Button 
							title={'Select USER IMAGES From Phone'}
							...
							onPress={async () => {
								try {
									...
									this.setState(prev => ({...prev, user_image: results}))
									// results.map((res) => {
					...
					<Button 
						title={'Press To Create Bulk Users'}
						style={styles.buttonWithoutBG}
						onPress={ () => {
							...
							Array.from(this.state.user_image).forEach((file) => {
								formData.append('user_image', {uri: file.uri, name: file.name, type: file.type})
							})
							formData.append('excel_sheet_for_user', {uri: this.state.excel_sheet.uri, name: this.state.excel_sheet.name, type: this.state.excel_sheet.type})
					...
					<View>
						<Button 
							title={'Press To DELETE ALL USERS'}
							style={styles.buttonWithoutBG}
							onPress={ () => {
