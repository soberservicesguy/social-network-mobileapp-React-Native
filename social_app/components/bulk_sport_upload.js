...
class BulkSportUpload extends Component {
	constructor(props) {
		super(props);
// STATE	
		this.state = {
			...
			// e.g a social post, textinput which lets user to enter text, takes persons id as assigned object
				<View style={styles.outerContainer}>
					...
					<View style={styles.textinputContainer}>
						<Button 
							title={'Select SPORT IMAGES From Phone'}
							...
							onPress={async () => {
								try {
									...
									this.setState(prev => ({...prev, sport_image: results}))
									// results.map((res) => {
					...
					<View style={styles.textinputContainer}>
						<Button 
							title={'Select SPORTS EXCEL SHEET From Phone'}
							style={styles.buttonWithoutBG}
							onPress={async () => {
								try {
					...
					<Button 
						title={'Press To Create Bulk SPORTS'}
						style={styles.buttonWithoutBG}
						onPress={ () => {
							...
							Array.from(this.state.sport_image).forEach((file) => {
								formData.append('sport_image', {uri: file.uri, name: file.name, type: file.type})
							})
							formData.append('excel_sheet_for_sport', {uri: this.state.excel_sheet.uri, name: this.state.excel_sheet.name, type: this.state.excel_sheet.type})
					...
					<View>
						<Button 
							title={'Press To DELETE ALL SPORTS'}
							style={styles.buttonWithoutBG}
							onPress={ () => {
...
export default BulkSportUpload